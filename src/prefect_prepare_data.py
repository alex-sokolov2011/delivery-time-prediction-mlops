...
def load_and_merge_data(config_path: str) -> pd.DataFrame:
    ...
    df = (
        orders_filtered.merge(
            items[['order_id', 'price', 'seller_id', 'product_id']], on='order_id'
        )
    ...
    output_path = os.path.join(
        cfg['prefect_root_data_dir'], 'prefect', 'merged_dataset.csv'
    )
...
def split_train_valid(path: str, config_path: str):
    ...
    train_path = os.path.join(
        cfg['prefect_root_data_dir'], 'prefect', 'train_dataset.csv'
    )
    valid_path = os.path.join(
        cfg['prefect_root_data_dir'], 'prefect', 'valid_dataset.csv'
    )

    df[(df[dt_col] >= train_start) & (df[dt_col] <= train_end)].to_csv(
        train_path, index=False
    )
    df[(df[dt_col] >= valid_start) & (df[dt_col] <= valid_end)].to_csv(
